...
static
int32_t comp_aatree_val(aatree_val a, aatree_val b)
{
  if (a > b){
    return 1;
  }
...
aatree_node *__aa_tree_insert__(aatree_val x, aatree_node *tree, int32_t *flag)
{
	int32_t cmp;
	*flag = 1;
	if (tree == &nil){
...
static
aatree_node *__aa_tree_delete__(struct del_info *info, aatree_node *tree, int32_t *flag)
{
	int32_t cmp;
	if (tree == &nil){
		return tree;
...
void aa_tree_print(aatree_node *tree, FILE *fp){
	...
	aa_tree_print(tree->left, fp);
	fprintf(fp, "%llx\n", tree->val);
	aa_tree_print(tree->right, fp);
}
static
